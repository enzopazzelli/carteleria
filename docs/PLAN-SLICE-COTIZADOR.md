...
6. **Desglose completo (`CART-308`).** `GET /presupuestos/{id}/desglose` devuelve cliente, líneas agrupadas por rubro (con override visible en cada línea) y los totales del paso 5 — todo en una sola respuesta, que es lo que la historia pide ("en una sola pantalla"), solo que la pantalla todavía no existe. Las piezas y el plano de cada grupo no viajan inline: ya existen `GET /trabajos/{id}/piezas` y `GET /ejecuciones/{id}/plano` para eso, así que alcanza con que cada línea `MATERIAL` lleve su `grupo_id`/`ejecucion_id` (nuevo en `LineaCosto`, y en `costeo.LineaMaterial` — otro campo que se calculaba adentro y se descartaba, mismo criterio que `precio_unitario` en el paso 2) para poder acceder "sin salir de la pantalla".
