...
function hasMaterialOverlap(A, B, config) {
  ...
  for (let i = 0; i < intersection.length; i++) {
    if (Math.abs(ClipperLib.Clipper.Area(intersection[i])) > AREA_MINIMA_CLIPPER) {
      return true;
    }
  }
...
function hasNonZeroClipperArea(paths) {
  for (let i = 0; i < paths.length; i++) {
    if (Math.abs(ClipperLib.Clipper.Area(paths[i])) > AREA_MINIMA_CLIPPER) {
      return true;
    }
  }
